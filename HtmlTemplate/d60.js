//    Comtrade analizator
//    Module: comtradeRio template d60 version 0.01
//    Alexander Voronenko
//    796546@gmail.com
//    09.2021

const version = "D60.v1_v0.01";
let textRio = "";
let zoneRioArray = [];
...
// структура общих уставок (вырез нагрузки)
const strucMain = {
   "RLOAD": "",
   "ANGLOAD": ""
}

// структура ДЗ уставок D60
const strucZ = {
   "ENABLEP": "",
   "DIRP": "",
   "SHAPEP": "",
...
function createZoneRioArray() {
   ...
   createStrucLoad(strucMain.RLOAD, strucMain.RLOAD, strucMain.ANGLOAD, 0, 11);
...
function createStrucZ(numZ) {
   ...
   for (let i = 0; i < param.length; i++) { strucZ[param[i].name.toUpperCase().slice(0, -1)] = param[i].value; }


   //  создаем массив с параметрами зон ДЗ zoneRioArray
   let nZone = zoneRioArray.length;
   // петля фаза-земля
   if (strucZ.ENABLEP == "TRUE") {
      zoneRioArray.push([]); //
      zoneRioArray[nZone].TYPE = "TRIPPING";
      zoneRioArray[nZone].FAULTLOOP = "LL";
      zoneRioArray[nZone].TRIPTIME = strucZ.TP;
      zoneRioArray[nZone].ACTIVE = "YES";
      zoneRioArray[nZone].INDEX = numZ;
      if (strucZ.SHAPEP == "QUAD") {
         zoneRioArray[nZone].SHAPE = "SHAPE";
         createQuadPP(nZone);
      } else {
   ...
   //петля фаза-фаза
   if (strucZ.ENABLEG == "TRUE") {
      zoneRioArray.push([]); //
      zoneRioArray[nZone].TYPE = "TRIPPING";
      zoneRioArray[nZone].FAULTLOOP = "LN";
      zoneRioArray[nZone].TRIPTIME = strucZ.TG;
      zoneRioArray[nZone].ACTIVE = "YES";
      zoneRioArray[nZone].INDEX = numZ;
      if (strucZ.SHAPEG == "QUAD") {
         zoneRioArray[nZone].SHAPE = "SHAPE";
         createQuadPE(nZone);
      } else {
         zoneRioArray[nZone].SHAPE = "MHOSHAPE";
...
function createQuadPP(curZone) {
   let nZ = 0;

   zoneRioArray[curZone].push([]);
   //  zoneRioArray[curZone][nZ] = createLine(strucZ., 0, linePhi, "LEFT"); nZ++;
}
