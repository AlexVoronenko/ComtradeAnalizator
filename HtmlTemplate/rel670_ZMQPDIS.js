...
function createFDPSPDIS(numZ) {
   ...
   if (strucZ.OPERATIONPE) {
      ...
      createZoneFDPSPDIS(Number(strucOkp.RFFWPE), Number(strucOkp.RFRVPP), Number(strucOkp.X1) + XN, 60, false);
      ++nZone;
   }
}
...
function createZonePP(curZone) {
   ...
   R1 = Number(strucZ.R1);
   X1 = Number(strucZ.X1);
   angle = radToDeg(Math.atan(R1 / X1));
   ...
   if (strucZ.DIR == "NONDIRECTIONAL") {
      ...
      zoneRioArray[curZone][nZ] = createLine(0, -X1, 0, "LEFT"); nZ++;
      zoneRioArray[curZone][nZ] = createLine(lineR, 0, 90, "LEFT"); nZ++;
   } else {
      // проверка на рисование линии левой 
      if (X1 * Math.tan(degToRad(30)) > lineR) {
         zoneRioArray[curZone][nZ] = createLine(-lineR, 0, 270, "LEFT"); nZ++;
      }
...
function createZonePE(curZone) {
   let nZ = 0;
   let calculate;
   let R1, lineR, lineR1, lineX, angle = 0;
   ...
   let X1 = Number(strucZ.X1);

   //   let R = Number(strucZ.Z) * Math.cos(degToRad(angle));

   let Xn = (Number(strucZ.X0) - Number(strucZ.X1)) / 3;
   let Rn = (Number(strucZ.R0) - Number(strucZ.R1)) / 3;
   ...
   //  calculate = radToDeg(Math.atan(lineX / lineR1));
   zoneRioArray[curZone][nZ] = createLine(lineR, 0, calculate.toFixed(3), "LEFT"); nZ++;
   zoneRioArray[curZone][nZ] = createLine(0, lineX, 180, "LEFT"); nZ++;
   if (strucZ.DIR == "NONDIRECTIONAL") {
      zoneRioArray[curZone][nZ] = createLine(-lineR, 0, 270, "LEFT"); nZ++;
      // calculate = radToDeg(Math.atan(lineX / lineR1));
